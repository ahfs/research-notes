...
**Problem 2: FP16 on MPS.** The original code used `torch.float16`, which produces silent numerical corruption on Apple Silicon's MPS backend. Scores come out as garbage.
...
> **Note on the original theft.mp4:** The file included in the repo is corrupt --- all frames decode as pure noise regardless of codec or decoder. We verified this with both OpenCV and FFmpeg. The synthetic theft video (time-reversed delivery) is a valid stand-in because the temporal structure is correct: package-present state transitions to package-absent state.
>
> {% include figure.liquid loading="eager" path="assets/img/blog/argus_blog_assets/theft_corrupt.jpg" title="example image" class="img-fluid rounded z-depth-1" %}
> *Every frame of theft.mp4 is pure static noise. Not a codec issue --- the data is genuinely corrupt.*
...
Time-reversing a delivery produces a perfect inverse. Real theft looks different: the person may approach from a different angle, move faster, look around nervously, or partially occlude the camera. The temporal symmetry of our test data flatters the algorithm.
